...
A etapa presente no repositório consiste na extração dos dados usando requests e salvo em uma camada Bronze (Raw) identica ao original, em seguinta realizamos a consolidação e tratamento das bases com SQL, PySpark e Pandas e salvo na camada Silver, na próxima etapa realizamos a aplicação de algumas regras definidas no escopo do projeto utilizando as mesmas tecnologias descritas e armazenado em uma camada Gold, por fim carregamos os dados no Google Drive via API para o seguimento das próximas etapas do projeto.
...
IDE: VS Code  
Formatos de arquivos: Parquet, CSV e JSON  
Outros: WSL
